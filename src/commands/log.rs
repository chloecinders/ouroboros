...
use chrono::Utc;
use serenity::{all::{ButtonStyle, Context, CreateActionRow, CreateButton, CreateEmbed, CreateInteractionResponse, CreateInteractionResponseMessage, CreateMessage, EditMessage, Message, Permissions}, async_trait};
...
impl Log {
    ...
    fn create_chunked_response(&self, chunk: &[LogRecord]) -> String {
        ...
        chunk.into_iter().for_each(|data| {
            ...
            let reason = if record.reason.chars().all(char::is_whitespace) || record.reason.is_empty() {
                String::new()
            } else {
                format!("```\n{}\n```\n", record.reason)
            };
            ...
            let update_string = if let Some(t) = record.updated_at {
                ...
                format!(" | At <t:{0}:d> <t:{0}:T>", record.created_at.and_utc().timestamp())
            };
...
impl Command for Log {
    ...
    async fn run(
        ...
        let mut page_buttons = vec![
            CreateButton::new("first").style(ButtonStyle::Secondary).label("<<").disabled(true),
            CreateButton::new("prev").style(ButtonStyle::Secondary).label("<").disabled(true),
            CreateButton::new("page").style(ButtonStyle::Secondary).label(format!("1/{}", chunks.len())).disabled(true),
            CreateButton::new("next").style(ButtonStyle::Secondary).label(">"),
        ...
        let mut log_buttons = vec![
            ...
            CreateButton::new("5").style(ButtonStyle::Secondary).label("5").disabled(chunk.get(4).is_none()),
        ];
        ...
        let get_updated_buttons = |page: usize, disabled: [bool; 4]| -> Vec<CreateButton> {
            ...
            new = new.into_iter().enumerate().map(|(i, mut d)| {
                if i == 2 {
        ...
        loop {
            let interaction = match msg.await_component_interaction(&ctx.shard).timeout(Duration::from_secs(60 * 5)).await {
                Some(i) => i,
                None => {
                    page_buttons = page_buttons.into_iter().map(|b| b.disabled(true)).collect();
                    log_buttons = log_buttons.into_iter().map(|b| b.disabled(true)).collect();
                    let _ = msg.edit(&ctx.http, EditMessage::new().components(vec![CreateActionRow::Buttons(page_buttons), CreateActionRow::Buttons(log_buttons)])).await;
                    return Ok(())
                },
            };
            ...
            match interaction.data.custom_id.as_str() {
                ...
                "1" | "2" | "3" | "4" | "5" => {
                    ...
                    let id = chunks.get(page).unwrap().get(log - 1).unwrap().id.to_string();
